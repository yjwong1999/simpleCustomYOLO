...
From the outputs, you can see the outputs of the selected backbone. Usually, you can select the last 3 (or 4 depending on your needs) outputs as the intermediate features for the neck layer of your YOLO. For this example, we are modifying YOLOv8. Hence, we only need 3 intermediate features/outputs (layer 6 to 8) from the outputs above. Then, we add `SPPF` layers, similar to YOLOv8.
```yaml
backbone:
